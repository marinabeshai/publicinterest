...
from utils.constants import Unknown, MORE_THAN_MILLION
import utils.ptr_utils as ptr_utils
import random 
from scipy.stats.mstats import gmean
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

def normalize_inner_dict(d, normalized, percent=False, raw=False):
    assert percent or raw 
    
    d_new = {}
    
    for k,inner_d in d.items():
        d_new[k] = normalize(inner_d, normalized, percent, raw)
...
def normalize(d, normalized, percent=False, raw=False):
    ...
    for k,v in d.items():
        ...
        if normalized[k] != 0: 
            percent = format(v/normalized[k], ".2f")

        if percent: 
            d_new[k + " (Normalized)"] = ptr_utils.commify_str(str(percent))
        
        if raw: 
            d_new[k + " (Raw)"] = ptr_utils.commify_str( (d[k])) 
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# d = increment_list_in_dictionary({}, 'marina', 5)
# d = {'marina': [5]}
#
# d = increment_list_in_dictionary(d, 'marina', 5)
# d = {'marina': [5, 5]}
#
# d = increment_list_in_dictionary(d, 'marina', 7)
# d = {'marina': [5, 5, 7]}
#
# d = increment_list_in_dictionary(d, 'marina', 3)
# d = {'marina': [5, 5, 7, 3]}
#
# d = increment_list_in_dictionary(d, 'jack', 5)
# d = {'marina': [5, 5, 7, 3], 'jack': [5]}
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def increment_list_in_dictionary(d, key, value):
    if key in d:
        d[key].append(value)
        return d 
    else:
...
def increment_set_in_inner_dictionary(d, key, inner_key, inner_value):
    try: 
        ...
        if key in d:
            d[key] = increment_set_in_dictionary(d[key], inner_key, inner_value)
...
def add_sort_key_for_amount(d, normal_header="", normal=False):
    ...
    try: 
        ...
        for amount in d:
            ...
            if amount == 'Over $50,000,000' or amount == '$50,000,000 +':
                d_copy[amount]["sort_key"] = 100000000
                continue

            if '+' in amount:            
                d_copy[amount]["sort_key"] = 1000000    
                continue 
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def sort_dictionary_by_tuple(d, which_value=1, reverse=True):
    
    def keyfunc(tup):
        l = list(tup)
        return l[which_value], list(d.keys())[0]
