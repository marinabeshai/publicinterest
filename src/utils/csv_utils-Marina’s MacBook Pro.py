...
from utils.constants import ALL_OF_THE_POSSIBLE_ENTRIES_FOR_AMOUNT, Unknown, AMOUNT
import csv
import os 
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
# 0  Sam      4       2424
# --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def make_csv_breakdown(path_csv, filename, d,  key_header, old_way=False):
    try:
        ...
        with open(wd, 'w') as csvfile:
            ...
            if not old_way and key_header == AMOUNT:
                values_2 = []
                for am in ALL_OF_THE_POSSIBLE_ENTRIES_FOR_AMOUNT: 
                    if am in values:
                        values_2.append(am)
                        continue 
                values = values_2
